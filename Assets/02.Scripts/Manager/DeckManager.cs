...
public class DeckManager : MonoBehaviour
{
    ...
    [Header("덱 존")]
    [SerializeField] private DeckZone deckZone;

    private Queue<HandCardControl> deck = new Queue<HandCardControl>();

    //private Dictionary<string, HandCardControl> _deck = new Dictionary<string, HandCardControl>();

    public Queue<HandCardControl> Deck
    {
        get { return deck; }
    }
    ...
    public void CreatDeck()
    {
        ...
        deckZone.DeckView(deck, deck.Count);
    }
    //카드 드로우!
    ...
    //다음 턴 시작 시 덱이 0인 경우(아직 턴 시스템을 못만들어서 확인 불가)
    public void DeckRelord()
    {
        if (deck.Count <= 0)
        {
            CreatDeck();
